...
public class StockTradeDao {
	...
	public void updateBuyMap(User user, Set<Trade> trades) {
		...
		if (u != null) {
			...
			for (Trade t : trades) {
				...
				logger.info("updateBuyMap:" + t.getStock().getTicker());
	...
	public void updateSellMap(User user, Set<Trade> trades) {
		...
		if (u != null) {
			...
			for (Trade t : trades) {
				long id = t.getStock().getId();
				logger.info("updateSellMap:" + t.getStock().getTicker());
	...
	public UserBank getBankFromUser(String email) throws NoResultException 
	{
		...
		TypedQuery<UserBank> query = this.entityManager
				.createQuery("SELECT a from UserBank a where a.user = ?1", UserBank.class);
		query.setParameter(1, u);
		//query.setMaxResults(1);
		logger.info("query result----->"+query.getSingleResult());
		return query.getSingleResult();
	}
	...
	public Trade createSellTrade(User user, Stock s, String quantity, Transaction trans) {
		...
		trade.setIndividualPrice(s.getPrice());
	...
	public Trade createBuyTrade(User user, Stock s, String quantity, Transaction transaction) {
		logger.info("INSIDE createBuyTrade::" + s.getTicker() + "  " + quantity);
